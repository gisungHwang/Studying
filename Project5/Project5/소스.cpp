...
void new_order(Menu* m, Order **ptr_array_order, int& pos)				// 주문 입력
{
	Order *o = new Order;
	bool search = false;
	...
	*(ptr_array_order + pos++) = o;		
	...
	cout << "주문이 등록되었습니다." << endl << endl;
	search = false;
}

void find_order(Order **ptr_array_order, int pos)				//주문 검색
{
	int table_num = 0;
...
void check_order(Order** ptr_array_order, int pos)			//주문 결제
{
	int table_num = 0;
	...
	for (int i = 0; i < pos; i++)
	{
		if (table_num == ptr_array_order[i]->table_num)
		{
			if (received_money >= ptr_array_order[i]->total_price + ptr_array_order[i]->total_price*0.1)		//받은 금액 체크
			{
...
void del_order(Order** ptr_array_order, int& pos)			//주문 삭제
{
	int table_num = 0;
	...
	for (int i = 0; i < pos; i++)
	{
		...
		{
			if (ptr_array_order[i]->isChecked == false)			//결제 체크
			{
