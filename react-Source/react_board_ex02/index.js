...
app.post('/minidetail', (req, res) => {
  console.log('/minidetail', req.body.comment_name);
  var num = parseInt(req.body.comment_name);
...
app.post('/miniupdate', (req, res) => {
  ...
  const sqlQuery =
    'UPDATE board_tbl SET comment_name =?, comment_content = ?, comment_price = ? WHERE BOARD_NAME = ?;';
  db.query(sqlQuery, [comment_name, comment_content, comment_price ], (err, result) => {
    res.send(result);
    console.log('Result = ', result);
  });
